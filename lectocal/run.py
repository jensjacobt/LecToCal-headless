...
def _get_arguments():
    parser = argparse.ArgumentParser(
        description="Scrapes a Lectio schedule " "and syncs it to Google Calendar."
    )
    parser.add_argument(
        "school_id", type=int, help="ID of the school user belongs to in Lectio."
    )
    parser.add_argument(
        "user_type",
    ...
    parser.add_argument("user_id", type=int, help="User's ID in Lectio.")
    parser.add_argument(
        "--login",
        default=False,
        dest="login",
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of weeks to parse the schedule for. " "(default: 4)",
    )
    parser.add_argument(
        "--showtop",
        default=False,
...
def sync(school_id, user_type, user_id, calendar_name, weeks, show_top, show_cancelled):
    """
    ...
    lectio_schedule = lectio.get_schedule(
        school_id, user_type, user_id, weeks, show_top, show_cancelled
    )
